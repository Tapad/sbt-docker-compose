...
case class OutputTable(table: List[List[String]]) {
  ...
  def mkBorder(borderSeparator: String, borderFiller: String): String = {
    columnWidthList.map(borderFiller * _.getOrElse(0)).mkString(s"$borderSeparator$borderFiller", s"$borderFiller$borderSeparator$borderFiller", s"$borderFiller$borderSeparator")
  }

  def mkRow(row: List[String], separator: String): String = {
